...
namespace gw2b {
	...
	PreviewGLCanvas::PreviewGLCanvas( wxWindow* p_parent, const int* p_attrib )
		: wxGLCanvas( p_parent, wxID_ANY, p_attrib, wxDefaultPosition, wxDefaultSize, wxFULL_REPAINT_ON_RESIZE )
		, m_reader( nullptr )
	...
	bool PreviewGLCanvas::previewFile( DatFile& p_datFile, const DatIndexEntry& p_entry ) {
		...
		if ( m_reader ) {
			...
			if ( isOfType<MapReader>( m_reader ) ) {
				...
				this->loadModel( p_datFile, model );
			}
	...
	void PreviewGLCanvas::clearShader( ) {
		m_shaders.clear( );
	}

	bool PreviewGLCanvas::isLightmapExcluded( const uint32& p_id ) {
		switch ( p_id ) {
	...
	void PreviewGLCanvas::loadModel( DatFile& p_datFile, const GW2Model& p_model ) {
	...
	bool PreviewGLCanvas::loadShader( ) {
	...
	void PreviewGLCanvas::initShaderValue( ) {
		if ( !m_shaders.empty( ) ) {
	...
	void PreviewGLCanvas::reloadShader( ) {
		this->clearShader( );
		if ( !this->loadShader( ) ) {
			this->clearShader( );
	...
	void PreviewGLCanvas::drawModel( Shader* p_shader, const glm::mat4& p_trans ) {
	...
	void PreviewGLCanvas::displayStatusText( ) {
		// Send ClientSize variable to text renderer
		m_text->setClientSize( m_clientSize );
	...
	void PreviewGLCanvas::updateMatrices( ) {
	...
	void PreviewGLCanvas::createFrameBuffer( ) {
		if ( m_statusAntiAlising ) {
